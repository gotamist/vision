...
class Naimish_style_net(nn.Module):
    ### https://arxiv.org/pdf/1710.00977.pdf
    # That paper has a different size for the input image, so there are some differences, but I have tried to follow it closely

    def __init__(self):
        super(Naimish_style_net, self).__init__()
        ...
        # 1 input image channel (grayscale), 32 output channels/feature maps, 5x5 square convolution kernel
        # Input is 1,224,224
        self.conv1 = nn.Conv2d(1, 32, 4) # if stride=1, the size is (32,221,221)
        I.kaiming_normal_(self.conv1.weight)
        self.conv1_bn = nn.BatchNorm2d(32)
        ...
        # pool with kernel_size=2, stride=2
        self.pool = nn.MaxPool2d(2, 2) #now it will be (32,110,110)
        self.conv2 = nn.Conv2d(32, 64, 3) # now it is (64, 108,108 )
        I.kaiming_uniform_(self.conv2.weight)
        self.conv2_bn = nn.BatchNorm2d(64)
        # After one more pool layer, it will be (64,54,54)
        self.conv3 = nn.Conv2d(64, 128, 2) # now it is (128, 54,54 ).  After one more pooling, it should be (128,26,26)
        I.kaiming_uniform_(self.conv3.weight)
        self.conv3_bn = nn.BatchNorm2d(128)
        self.conv4 = nn.Conv2d(128, 256, 1)# now it is (256, 26,26 ).  After one more pooling, it should be (256,13,13)
        I.kaiming_uniform_(self.conv4.weight)
        self.conv4_bn = nn.BatchNorm2d(256)
